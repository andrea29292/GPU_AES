...
/*Implementacion de la SBOX invertida*/
const uChar INV_SBOX[256] = {
...
int tot_rounds = 11;
int rows = 4; //righe della matrice stato
int columns = 4; //colonne della matrice stato
	...
	for (int i = 0; i < columns; i++) {
		for (int j = 0; j < rows; j++) {
			if (plain_text[k] == NULL) {
				...
				state[j][i] = getVal(plain_text[k]) * 16 + getVal(plain_text[k + 1]);
			}
			k += 2;
		}
	}
...
uChar* shiftRow(uChar* row, int row_num) {
	uChar* new_row = malloc(columns * sizeof(uChar*));
	int new_c;
...
void subBytes(uChar** state) {
	...
	for (state_r = 0; state_r < rows; state_r++) {
		for (state_c = 0; state_c < columns; state_c++) {

			sbox_r = (state[state_r][state_c] & 0xf0) >> 4;
			sbox_c = state[state_r][state_c] & 0x0f;

			state[state_r][state_c] = SBOX[sbox_r * 16 + sbox_c];
...
void gmixColumn(int c) {
	uChar new_column[4];
	new_column[0] = GM2[state[0][c]] ^ GM3[state[1][c]] ^ state[2][c] ^ state[3][c];
	new_column[1] = state[0][c] ^ GM2[state[1][c]] ^ GM3[state[2][c]] ^ state[3][c];
	new_column[2] = state[0][c] ^ state[1][c] ^ GM2[state[2][c]] ^ GM3[state[3][c]];
	new_column[3] = GM3[state[0][c]] ^ state[1][c] ^ state[2][c] ^ GM2[state[3][c]];
	for (int r = 0; r < rows; r++) {
		state[r][c] = new_column[r];
	}
}
...
void plot() {
	...
	if (gnuplotPipe) {
		...
		//fprintf(gnuplotPipe, "plot 'scores.dat' using 1:2 title 'CPU' with linespoints");
		fflush(gnuplotPipe);
...
int main(int argc, char** argv) {
	...
	test_sizes[5] = 31134208/4;
	test_sizes[6] = 31134208/2;
	test_sizes[7] = 31134208;
	test_sizes[8] = 31134208*2;
